        ...
        Group {
            if UIDevice.current.userInterfaceIdiom == .pad
            {
                GeometryReader { proxy in
                    ...
                    VStack(spacing: 0) {
                        HStack(spacing: 0) {
                            ControlsView().frame(width: 380).layoutPriority(1).background(Color(.secondarySystemBackground))
                        ...
                        }.frame(width: proxy.size.width, alignment: .center).background(Color(.secondarySystemBackground))
...
struct ControlsView: View {
    ...
    var body: some View {
        ...
        }.padding(10).background(Color.secondary)
    }
}
